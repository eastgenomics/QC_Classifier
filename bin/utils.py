"""
Personal set of functions which are used to complete some of the main steps
...
- get_unique_parameters(unique_id, yaml_file):
    - Function  that outputs the list of conditions/parameters found in the
    config.yaml file.
...
def map_header_id(config_field) -> str:
    ...
    Returns:
        str: Header ID, it is the associated metric used in the multiqc.json
        file
    """
    # First get file from resources/ folder
    config_header_filepath = Path(__file__).parent.joinpath(
        "..", "resources", "idConfigFieldRelationship.json"
    )
    with open(config_header_filepath, "r", encoding="UTF-8") as file:
        config_field_relationship = json.load(file)

    id_mapping = {
        item["Config_field"]:
        item["Header_ID"] for item in config_field_relationship
    }
...
def read_config(yaml_file) -> dict:
    ...
    if not yaml_file.get("table_cond_formatting_rules"):
        raise TypeError(
            f"{yaml_file} does not have " + "'table_cond_formatting_rules' key"
        )
...
def get_unique_parameters(unique_id, yaml_file) -> dict:
    ...
    Input:
        - Config field (str): fields found in table_cond_formatting_rules from
        the "config.yaml"
                              file.
...
def get_sample_metric_value(summarised_data, sample_id, header_id):
    """Return the value of given sample ID and header_id in a dictionary
    format.
...
def get_status(value, parameters):
    ...
    for possible_status in list(parameters.keys()):
        ...
        # Iterate through the list of conditions for each possible status:
        # 'gt', 'lt', 'eq', 's_eq'
        for condition in conditions:

            # Checking if any of the following conditions exist
            # The "or statements" are necessary to prevent any condition with
            # value 0 to be treated as false
        ...
        # Config field "Match_Sexes" may return value as a string "false" or
        # "true" which is different to what is set for the config fields.
...
def get_output_filename(summarised_data):
    ...
    """
    match = re.search(
        "[0-9]{6}_[A-Z0-9]{6}_[0-9]{4}_[A-Z0-9a-z_-]+multiqc",
    ...
    if match:
        ...
        print(
            "Run ID could not be parsed from 'report_multiqc_command' field \
                in multiqc json:\n"
